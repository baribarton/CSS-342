...
DList<Object>::DList(const DList &rhs) {
	...
	cost = 0; 
	*this = rhs;
	// then assign rhs to this.
}
template<class Object>
DList<Object>::~DList() {
	clear();
	// delete items starting 1st 
	delete header;                               
	// delete a dummy (at 0th).
}
template<class Object> bool DList<Object>::isEmpty() const {
...
int DList<Object>::size() const {
	...
	// postfix ++ doesn't count   
	return size;
	// dummy
}
template<class Object> void DList<Object>::clear() {
	while (!isEmpty()) {
		DListNode<Object> *node = findByIndex(1);
		// find the 1st object   
		remove(node->item);
		// remove it 
	}
...
int DList<Object>::find(const Object &obj) {
	...
	int i = 0;   for (; p != NULL && p->item != obj; p = p->next, ++i)
		++cost;
...
template<class Object> int DList<Object>::getCost() const {
	return cost;
}
template<class Object> DListNode<Object>
	*DList<Object>::findByIndex(const int index) const {
